...
== Differentiation Methods ==
Finding the gradient of the loss requires a differentiation algorithm. There are three algorithms/methods that help us find the derivatives:

. Symbolic differentiation
. Numerical differentiation
. Automatic differentiation

Let's see these methods with an example.
...
There are three methods that help us find these derivatives.

=== Symbolic Differentiation ===
Symbolic differentiation finds the derivatives by directly manipulating the mathematical expression. It treats functions as expressions, and applies algebraic rules (e.g., chain rule, product rule) to manipulate those expressions into new symbolic derivatives.

This method can find the exact derivative, but depending on the shape of stem:[L], the computation becomes very complex and time-consuming. For deeply nested function compositions (like in deep networks), this leads to exponential growth in expression size. They aren't designed for efficient evaluation (no reuse of intermediate computations) - they are for algebraic manipulation.
...
=== Automatic Differentiation ===
Automatic differentiation allows us to obtain the accurate gradients of the loss with respect to each parameter in neural networks. There are two methods of automatic differentiation: 

*Forward Mode AD:* Computes the partial derivatives during the forward pass operation. It is slow, and not suitable for gradient-based optimization. To carry out this method, let's look at the computational graph representing the evaluation of the given function:
...
Computes the partial derivatives during the backward pass, i.e., propagates derivatives backward from the output. This mode is faster than forward mode. This mode is used to train the neural networks.
