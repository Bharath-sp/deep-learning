...
== Introduction ==
There is a sequential nature to RNNs (and its variants). Suppose the length of the input sequence is stem:[n]. RNN runs through the time steps sequentially, so it needs stem:[\mathbb{O}(n)] steps to process this sentence. We cannot process all the steps parallely as each time step is recursively dependent on the previous step output. This is a bottleneck. And RNNs also use attention to deal with long-range dependencies.

Since attention enables access to *any* encoder hidden state during decoding, do we still need RNNs? It seems like we can get rid of them.

NOTE: With attention mechanism, we access the encoder hidden states not as a sequence, but as a set (without respecting their temporal nature).

Transformer models are the models that are solely based on attention mechanisms, dispensing with recurrence and convolutions entirely. These models are known as transformers because they transform a set of vectors in some representation space into a corresponding set of vectors, having the same dimensionality, in some new space, i.e., the dimension of the space is the same, but the subspace is different. The vectors are transformed from a subspace to another new subspace. The goal of the transformation is that the new subspace will have a richer internal representation that is better suited to solving downstream tasks.

Transformers were originally introduced in the context of natural language processing, but have subsequently been found to achieve excellent results in many other domains.

== What is Attention? ==
The fundamental concept that underpins a transformer is attention. Let's motivate the use of attention using natural language as an example, although it has much broader applicability.
...
Suppose each input vector corresponds to an user. In this transformation, we project the input vectors from one space into another input space. For example, the input vectors may be of size 1000 with 1 and 0s representing whether an user has watched a particular movie or not (movies watched space). We transform this vector into a new space, i.e., to a vector of the same size, but with each element representing users' taste (taste space).

We can then apply multiple transformer layers (encoder blocks) in succession to construct deep networks capable of learning rich internal representations.
...
Each word in the input sequence is transformed to a fixed-size vector. These vectors are fed into the bottom-most encoder. Other encoders receive updated representation vectors from the encoder immediately below it. The final outputs from the encoder are passed to the decoder. The decoder decodes and generates the output sequence.
...
+
The outputs of this layer are vector representations of the time series data. The size of the vector is a hyperparameter stem:[D_k].
